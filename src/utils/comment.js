...
async function addComment(req,res){
    try{
        var comment = await pool.query("INSERT INTO comments(post_id,comment_id,user_id,likes,comment,username,profile_pic,time,created) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)",[
            req.body.post_id,
            req.body.comment_id,
            req.body.user_id,
            req.body.likes,
...
async function getComments(req,res){
    try{
        const comments = await pool.query("SELECT * from comments where post_id=$1  AND (time>$2 OR time<$3) ORDER BY time DESC limit 9",[
            req.body.post_id,
            req.body.upperLimit,
            req.body.lowerLimit
        ])
...
async function likeComment(req,res){
    try{
        const comments = await pool.query("INSERT INTO commentLikes(id) VALUES($1)",[
            `${req.body.comment_id}-${req.body.user.id}`
        ])
        const inc = await pool.query("UPDATE comments SET likes=likes+1 WHERE comment_id=$1",[
            req.body.comment_id
        ])
